...
exports.makeVerificationDecision = makeVerificationDecision;
 ...
 */
function makeVerificationDecision(pipeline, config) {
    const reasons = [];
    let decision = 'verified';
    // ─── Hard Rejection Criteria ─────────────────────────────
    // Fraud: duplicate screenshot is an instant rejection
    const hasDuplicate = pipeline.fraudDetection.signals.some((s) => s.type === 'duplicate_screenshot' && s.detected);
    if (hasDuplicate) {
        reasons.push('This payment screenshot has been previously submitted.');
        decision = 'rejected';
    }
    // Fraud: editing software detected
    const hasEditingSoftware = pipeline.fraudDetection.signals.some((s) => s.type === 'editing_software' && s.detected);
    ...
    logger_1.logger.info({ decision, overallConfidence, reasonCount: reasons.length }, 'Verification decision made');

...
exports.runTransactionValidation = runTransactionValidation;
...
function runTransactionValidation(fields) {
    try {
        ...
        if (fields.transactionId) {
            const matchesKnownFormat = Object.values(knownIdFormats).some((pattern) => pattern.test(fields.transactionId));
            if (!matchesKnownFormat) {
                logger_1.logger.debug({ transactionId: fields.transactionId }, 'Transaction ID does not match any known format');
        ...
        logger_1.logger.debug({ statusText, isSuccessful, formatValid, passed }, 'Transaction validation complete');
        return {
            statusText: fields.status,
            isSuccessful,
            formatValid,
        ...
        logger_1.logger.error({ error }, 'Transaction validation failed');
        return {
            isSuccessful: false,
            formatValid: false,
            passed: false,

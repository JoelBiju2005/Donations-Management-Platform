 ...
 */
async function checkExifMetadata(imageBuffer) {
    try {
        const exif = await exifr_1.default.parse(imageBuffer, { pick: ['Software', 'CreatorTool', 'ModifyDate', 'CreateDate'] });
        if (!exif) {
...
async function checkErrorLevelAnalysis(imageBuffer) {
    try {
        ...
        const original = await (0, sharp_1.default)(imageBuffer).greyscale().raw().toBuffer({ resolveWithObject: true });
        const recompressed = await (0, sharp_1.default)(imageBuffer)
            .jpeg({ quality: 20 })
            .greyscale()
            .raw()
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (!regionDiffs[regionIdx])
                regionDiffs[regionIdx] = 0;
        ...
        logger_1.logger.debug({ error }, 'ELA check failed');
        return { type: 'ela_error', description: 'ELA analysis could not be performed', weight: 0, detected: false };
    }
}
async function checkDuplicateHash(screenshotHash, currentDonationId) {
    if (!screenshotHash) {
    ...
    try {
        ...
        const duplicate = await Donation_1.Donation.findOne({
 ...
 */
async function runFraudDetection(imageBuffer, screenshotHash, donationId, fraudThreshold) {
    ...
    logger_1.logger.debug({ riskScore, fraudThreshold, flagged, detectedCount: detectedSignals.length }, 'Fraud detection complete');
    return {
        riskScore,
        signals,
        flagged,

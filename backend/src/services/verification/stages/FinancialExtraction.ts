import { ExtractedFields, FinancialExtractionResult } from '../types';
...
export function runFinancialExtraction(ocrText: string): FinancialExtractionResult {
  try {
    ...
    logger.debug({ fieldsExtracted, confidence, fields }, 'Financial extraction complete');
    ...
    logger.error({ error }, 'Financial extraction failed');
    return {
      fields: {},
      confidence: 0,
      passed: false,
